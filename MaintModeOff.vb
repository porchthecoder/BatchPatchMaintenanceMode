...
Module Module1
	...
	Const ApplicationName As String = "MaintModeOff"
	...
	Sub Main()
		...
		Dim flag As Boolean = Not File.Exists("c:\batch\service_list_mm_off.txt")
		If flag Then
			ServiceError = 255
			'Console.WriteLine("ERROR c:\batch\service_list_mm_off.txt does not exist")
			pWriteStatus("ERROR c:\batch\service_list_mm_off.txt does not exist", EventLogEntryType.Error)
		Else
			For Each myService As String In File.ReadAllLines("c:\batch\service_list_mm_off.txt")
				Dim flag2 As Boolean = myService.Length > 1
				If flag2 Then
					'Console.WriteLine("Starting:" + myService)
					pWriteStatus("Starting:" + myService, EventLogEntryType.Information)
	...
	Private Sub StartService(ServiceName As String)
		...
		Try
			Process.Start("sc", "config """ + ServiceName + """ start= auto")
		Catch ex As Exception
			'Console.WriteLine("ERROR " + ex.Message + " when enabling service " + ServiceName)
			pWriteStatus("ERROR " + ex.Message + " when enabling service " + ServiceName, EventLogEntryType.Error)
			ServiceError = 1
			Return
		End Try
		Thread.Sleep(5000)
		Try
			Dim flag As Boolean = serviceController.Status.Equals(ServiceControllerStatus.Running)
			If flag Then
				'Console.WriteLine(ServiceName + " is already started")
				pWriteStatus(ServiceName + " is already started", EventLogEntryType.Information)
				Return
			End If
		...
		Catch ex3 As Exception
			'Console.WriteLine("ERROR " + ex3.Message + " when starting " + ServiceName)
			pWriteStatus("ERROR " + ex3.Message + " when starting " + ServiceName, EventLogEntryType.Error)
			ServiceError = 1
			Return
		End Try
		Thread.Sleep(10000)
		...
		Do
			Dim flag2 As Boolean = serviceController.Status.Equals(ServiceControllerStatus.Running)
			If flag2 Then
				Return
			End If
			'Console.WriteLine("Wating for " + ServiceName + " to start")
			pWriteStatus("Wating for " + ServiceName + " to start", EventLogEntryType.Information)
			Thread.Sleep(60000)
			serviceController.Refresh()
			num += 1
			flag3 = (num = 5)
		Loop While Not flag3
		'Console.WriteLine("Timeout wating for " + ServiceName + " to start")
		pWriteStatus("Timeout wating for " + ServiceName + " to start", EventLogEntryType.Error)
		ServiceError = 1
	End Sub
